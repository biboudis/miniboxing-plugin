package miniboxing.benchmarks.streams.specialized
...
final class Stream[@specialized T: ClassTag](val streamf: (T => Boolean) => Unit) {
  ...
  def map[@specialized R: ClassTag](f: T => R): Stream[R] =
  ...
  def flatMap[@specialized R: ClassTag](f: T => Stream[R]): Stream[R] =
    new Stream(iterf => streamf(value => {
  ...
  def foldLeft[@specialized A](a: A)(op: (A, T) => A): A = {
    var acc = a
    streamf(value => {
      acc = op(acc, value)
      true
  ...
  def sum[@specialized N >: T](implicit num: Numeric[N]): N =
    foldLeft(num.zero)(num.plus)
}

object Stream {
  @inline def apply[@specialized T: ClassTag](xs: Array[T]) = {
